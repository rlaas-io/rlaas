...
class RlaasClient:
    ...
    def check_limit(self, req: CheckRequest) -> Decision:
        data = self._post("/v1/check", req.to_dict())
        return Decision(
    ...
    def acquire(self, request: Dict[str, Any]) -> Dict[str, Any]:
        return self._post("/v1/acquire", request)

    def release(self, lease_id: str) -> Dict[str, Any]:
        return self._post("/v1/release", {"lease_id": lease_id})
    ...
    def create_policy(self, policy: Policy) -> Dict[str, Any]:
        return self._post("/v1/policies", policy.to_dict())

    def update_policy(self, policy_id: str, policy: Policy) -> Dict[str, Any]:
        return self._put(f"/v1/policies/{policy_id}", policy.to_dict())

    def delete_policy(self, policy_id: str) -> None:
        self._delete(f"/v1/policies/{policy_id}")

    def validate_policy(self, policy: Policy) -> Dict[str, Any]:
        return self._post("/v1/policies/validate", policy.to_dict())

    def update_rollout(self, policy_id: str, rollout_percent: int) -> Dict[str, Any]:
        return self._post(f"/v1/policies/{policy_id}/rollout", {"rollout_percent": rollout_percent})

    def rollback_policy(self, policy_id: str, version: int) -> Dict[str, Any]:
        return self._post(f"/v1/policies/{policy_id}/rollback", {"version": version})

    def list_policy_audit(self, policy_id: str) -> List[Dict[str, Any]]:
        return self._get(f"/v1/policies/{policy_id}/audit")

    def list_policy_versions(self, policy_id: str) -> List[Dict[str, Any]]:
        return self._get(f"/v1/policies/{policy_id}/versions")

    def analytics_summary(self, top: Optional[int] = None) -> Dict[str, Any]:
        query = ""
        if top is not None:
            query = f"?top={top}"
        return self._get(f"/v1/analytics/summary{query}")
